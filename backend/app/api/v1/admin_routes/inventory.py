...
@router.post("/materials/{material_id}/stock-logs", response_model=ApiResponse[dict[str, Any]])
...
@router.post("/materials/{material_id}/loss", response_model=ApiResponse[dict[str, Any]])
def create_material_loss(material_id: int, payload: MaterialLossCreate, _: dict = Depends(require_admin)):
...
def release_lock(lock_id: int, _: dict = Depends(require_admin)):
...
def consume_lock(lock_id: int, _: dict = Depends(require_admin)):
    return success_response({"id": lock_id, "status": "consumed"})


@router.get("/finished-goods", response_model=ApiResponse[PageResponse[dict[str, Any]]])
